...
# Create positions in dataframe

def check_positions(df, tech_list, op_list, package_list):
    for tech in tech_list:
        df['{}'.format(tech)] = df['description'].apply(lambda x: 1 if tech.upper() in x.upper() else 0)
    for op in op_list:
        df['{}'.format(op)] = df['description'].apply(lambda x: 1 if op.upper() in x.upper() else 0)
    for pack in package_list:
        df['{}'.format(pack)] = df['description'].apply(lambda x: 1 if pack.upper() in x.upper() else 0)
    return df
...
def create_chart_from_dict(dict_object, df):
    ...
    new_df['percentage'] = round(new_df['positions'] / sum(new_df['positions']) * 100, 2)
    new_df = new_df[new_df['percentage'] > 0].sort_values(by = 'percentage', ascending = False)
    if dict_object == tech:
        title = 'Technology'
        ...
        title = 'Package'
    fig = px.bar(new_df, x = 'item', y = 'percentage', 
                     color = 'item',
...
def create_donut(df, edu_level):
    for level in edu_level:
        df['{}'.format(level)] = df['description'].apply(lambda x: 1 if level.upper() in x.upper() else 0)
    labels = ['Bachelors', 'PhD', 'Masters']
    values = [df.bachelor.sum(), df.phd.sum(), df.master.sum() + df.msc.sum()]
    fig = px.pie(names = labels, values = values, hole = .6, 
                 color_discrete_sequence=px.colors.sequential.Viridis,
...
def create_map_data(df):
    ...
    proportion = pd.DataFrame(df.new_places.value_counts(normalize = True))
    proportion = proportion.reset_index()
    proportion = proportion.rename(columns = {'index': 'name', 'new_places': 'proportion'})
    map_df = vietnam.merge(proportion, how = 'left', on = 'name')
    map_df = map_df[['id', 'name', 'geometry', 'proportion']]
    map_df = map_df.to_crs(epsg=4326) # convert the coordinate reference system to lat/long
    return map_df
...
def create_map(map_df):
    ...
    layout = go.Layout(
        title = {'text': f"<b>Spread of positions</b> across <b>Vietnam</b>",
                 'xanchor': 'left'},
            mapbox1 = dict(
                domain = {'x': [0, 1],'y': [0, 1]}, 
